...
Entrega, Envio e Pagamento. Termo canônico desde `ADR-002`/`ADR-003` — substitui "Ciclo
Mensal", que é **termo banido**. *(`CONTRATO_SOBERANO.md` §4/§6.3; `ADR-002`)*
...
|---|---|---|
| `Ciclo` / `Ciclo Mensal` | `Colaboração Mensal` | `CONTRATO_SOBERANO.md` §2; `ADR-002` |
| `Plano de Colaboracao` | (sem substituto único — decompor em Briefing/Entrega/Envio/Pagamento) | `CONTRATO_SOBERANO.md` §2 |
| `Campanha` (como sinônimo do agregado mensal) | `Colaboração Mensal` | `ADR-002` — nota: "Campanha" reaparece com outro sentido no vocabulário do Sistema B, ver seção seguinte |
...
|---|---|---|
| `Campanha` | (sem equivalente direto — mais próximo de um agrupamento acima de `Colaboração Mensal`) | `Campanha` tem ciclo de vida próprio (`PLANEJADA→ATIVA→ENCERRADA|CANCELADA`) que `Colaboração Mensal` não tem |
